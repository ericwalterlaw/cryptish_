...
const Dashboard = ({ user }) => {
  ...
  const fetchTopCryptos = async () => {
    try {
      ...
      // Note: This is an external API call, so we use fetch directly
      ...
      setTopCryptos([
        ...
        { id: 'ethereum', name: 'Ethereum', symbol: 'ETH', current_price: 3200, price_change_percentage_24h: -1.8 },
        { id: 'binancecoin', name: 'BNB', symbol: 'BNB', current_price: 320, price_change_percentage_24h: 0.9 }
      ])
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-primary-600 to-primary-700 rounded-xl p-6 text-white">
        ...
        <p className="text-primary-100">Here's what's happening with your crypto portfolio</p>
      </div>
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">${(stats.balance + stats.portfolioValue).toFixed(2)}</p>
            </div>
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-white">${stats.portfolioValue.toFixed(2)}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              recentTransactions.map((tx, index) => (
                <div key={index} className="flex items-center justify-between p-4 bg-gray-700 rounded-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <p className="text-white font-medium">{tx.type.toUpperCase()} {tx.symbol}</p>
                      <p className="text-gray-400 text-sm">{new Date(tx.date).toLocaleDateString()}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-white font-medium">{tx.amount} {tx.symbol}</p>
                    <p className="text-gray-400 text-sm">${tx.value.toFixed(2)}</p>
                  </div>
                </div>
              ))
            ) : (
        ...
        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
          <h2 className="text-xl font-semibold text-white mb-4">Top Cryptocurrencies</h2>
          <div className="space-y-4">
            {topCryptos.map((crypto) => (
              <div key={crypto.id} className="flex items-center justify-between p-4 bg-gray-700 rounded-lg hover:bg-gray-600 transition-colors">
                <div className="flex items-center space-x-3">
                ...
                <div className="text-right">
                  <p className="text-white font-medium">${crypto.current_price?.toLocaleString()}</p>
                  <p className={`text-sm ${
